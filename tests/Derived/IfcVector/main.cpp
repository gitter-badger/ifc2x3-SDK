...
int main(int n, char **p)
{
	...
	Step::RefPtr< ifc2x3::IfcVector > Vector = eds->createIfcVector();
	TEST_VALIDITY(Vector);

	TEST_ASSERT(Vector->getDim()==0);

	Step::RefPtr< ifc2x3::IfcDirection > Orientation = eds->createIfcDirection();
	Orientation->getDirectionRatios().push_back(1);
	Orientation->getDirectionRatios().push_back(1);

	Vector->setOrientation(Orientation);
	Vector->setMagnitude(1);
	TEST_ASSERT(Vector->getDim()==2);
	
	Orientation->getDirectionRatios().push_back(1);
	Vector->setOrientation(Orientation);
	TEST_ASSERT(Vector->getDim()==3);
